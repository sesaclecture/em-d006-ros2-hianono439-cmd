...
def make_ros2_pkg_create_command(package_name):
    return f"ros2 pkg --build-type ament_python create {package_name}"
...
def make_ros2_run_command(package_name, node_name):
    return f"ros2 run {node_name} {package_name}"
...
def make_ros2_launch_command(package_name, launch_file):
    return f"ros2 launch {launch_file} {package_name}"
...
def make_ros2_docker_run_command(
    ...
    return f"docker run -dit --name:{container_name} --network host DISPLAY=$DISPLAY /tmp/.X11-unix:/tmp/.X11-unix {image_name}"
